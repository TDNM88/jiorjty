...
export function ModernChart({ 
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (typeof chart.addCandlestickSeries === 'function') {
        ...
        // Fallback to using the series API directly if available
        // @ts-ignore - Using internal API as fallback
        const series = chart.addSeries && chart.addSeries('Candlestick', {
          upColor: '#26a69a',
          downColor: '#ef5350',
          borderVisible: false,
          wickUpColor: '#26a69a',
        ...
        if (series) candleSeries = series;
      }
      
      if (!candleSeries) {
        throw new Error('Failed to create candlestick series: Method not available');
      }
    } catch (error) {
    ...
    if (showVolume) {
      try {
        if (typeof chart.addHistogramSeries === 'function') {
          ...
          // Fallback to using the series API directly if available
          // @ts-ignore - Using internal API as fallback
          const series = chart.addSeries && chart.addSeries('Histogram', {
            color: theme === 'dark' ? 'rgba(120, 120, 120, 0.5)' : 'rgba(120, 120, 120, 0.2)',
            priceFormat: { type: 'volume' },
            priceScaleId: '',
          });
          if (series) volumeSeries = series;
        }
        
        if (volumeSeries) {
          const priceScale = (volumeSeries as any).priceScale && (volumeSeries as any).priceScale();
    ...
    try {
      ...
      console.error('Failed to set up resize observer:', error);
      window.addEventListener('resize', handleResize);
    }
  ...
  }, [theme, height, showVolume, data.length]);
